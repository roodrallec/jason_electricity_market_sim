...
+!buyer(Agent, E) : seller(Seller,Sell,S) & sact(S)  <-
...
+!pairBuyer(S_agent, N) :true <- true.
	
+!pairSeller(S_agent, N) : buyer(B_agent, E_buying, N) & seller(S_agent, E_selling, A) & bact(B)  <-
	-+bact(B+1);
	//.print("pairing buyer agent ", B_agent, " with seller agent ", S_agent);
	.print("pairing buyer agent ",B_agent,"  ", E_buying, " with seller agent ",S_agent,"  ", E_selling);
	.min([E_selling, E_buying], AgreedAmount);
...
+!pairSeller(S_agent, N) :true <- true.
